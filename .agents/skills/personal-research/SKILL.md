...
3. If you need a starting source set:
   - `scripts/run.sh search-web --run-id "<id>" --query "<query>" --max-results 5`
4. Research with web/browser tools.
5. Prefer captured sources when possible:
   - `scripts/run.sh capture-url --run-id "<id>" --url "<url>" --notes "<why it matters>"`
6. If a source cannot be captured but should still be tracked:
   - `scripts/run.sh add-source --run-id "<id>" --url "<url>" --title "<title>" --notes "<why it matters>"`
7. For each meaningful finding:
   - `scripts/run.sh add-claim --run-id "<id>" --claim "<claim>" --confidence "<0-1>" --status "<tentative|verified|contradicted>" --source-url "<url>"`
8. Add follow-up tasks when useful:
   - `scripts/run.sh add-task --run-id "<id>" --task "<next action>"`
9. Close when enough evidence exists:
